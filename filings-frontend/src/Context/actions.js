...
export const getRequests = async (dispatch) => {
  const result = await fetch("http://127.0.0.1:8000/api/v1/req-data-all");
...
export const getUsers = async (dispatch) => {
  const result = await fetch("http://127.0.0.1:8000/api/users-data-all");
  const content = await result.json();
  if (content) {
...
export const cmdgetRequests = async (dispatch) => {
  const cmddata = await axios.get(
    "http://127.0.0.1:8000/api/v1/job-support-comment-data-all"
  );
