...
const UpdateLogics = ({ params, page }) => {
  ...
  const editData = () => {
    axios
      .put(`http://localhost:8000/api/v1/job-supp-table-admin-update`, editedData)
      .then((res) => {
  ...
  const paymentRequests = () => {
    if (paymentsLength === 0) {
      return axios
        .post(
          `http://localhost:8000/api/v1/job-supp-table-admin-payment-all`,
          paymentData
        )
        .then((res) => {
          console.log(res.data);
      ...
      return axios
        .put(
          `http://localhost:8000/api/v1/job-supp-table-admin-payment-update`,
          paymentUpdateData
        )
        .then((res) => {
          console.log(res.data);
